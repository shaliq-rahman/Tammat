...
class CategoryFieldController extends PanelController
{
	...
	public function setup()
	{
		...
		*/
		...
		// Category => CategoryField
		if ($this->parentEntity == 'categories') {
			$this->xPanel->setRoute(config('larapen.admin.route_prefix', 'admin') . '/categories/' . $category->id . '/custom_fields');
		...
		// Field => CategoryField
		if ($this->parentEntity == 'custom_fields') {
			$this->xPanel->setRoute(config('larapen.admin.route_prefix', 'admin') . '/custom_fields/' . $field->id . '/categories');
			$this->xPanel->setEntityNameStrings(
