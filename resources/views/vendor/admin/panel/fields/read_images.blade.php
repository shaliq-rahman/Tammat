		...
		@foreach ($entity_model as $connected_entity_entry)
			<div style="margin: 10px 5px; display: inline-block;">
				...
				<div style="text-align: center; margin-top: 10px;">
					<a href="{{ url(config('larapen.admin.route_prefix', 'admin') . '/pictures/' . $connected_entity_entry->id . '/edit') }}" class="btn btn-xs btn-default"><i class="fa fa-edit"></i> {{ trans('admin::messages.Edit') }}</a>&nbsp;
					<a href="{{ url(config('larapen.admin.route_prefix', 'admin') . '/pictures/' . $connected_entity_entry->id) }}" class="btn btn-xs btn-default" data-button-type="delete"><i class="fa fa-trash"></i> {{ trans('admin::messages.Delete') }}</a>
				</div>
			</div>
		@endforeach
        @if ($entity_model->count() < $posts_pictures_number)
            <hr><br>
            <a href="{{ url(config('larapen.admin.route_prefix', 'admin') . '/pictures/create?post_id=' . Request::segment(3)) }}" class="btn btn-xs btn-default"><i class="fa fa-edit"></i> {{ trans('admin::messages.add') }} {{ trans('admin::messages.picture') }}</a><br><br>
        @endif
	@else
