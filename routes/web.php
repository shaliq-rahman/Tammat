...

{
    ...
    return $response;
		...
		Route::get('/andriodCloseAccount', 'Auth\RegisterController@showCloseAccount');

		Route::post('/andriodCloseAccount', 'Auth\RegisterController@saveCloseAccount');
		...
		Route::group(['middleware' => ['auth', 'bannedUser', 'preventBackHistory'], 'namespace' => 'Account'], function ($router) {
			...
			Route::get('generalSettings', 'EditController@generalSettings');
		...
		Route::post('ajax/messages/check', 'Ajax\ConversationController@checkNewMessages');		
			...
			Route::get('account/{pagePath}/{id}/archivepost', 'PostsController@archivepost');//check that 2-9-2020	 
			...
			Route::get('account/recharge_points', 'RechargeController@index');

			Route::post('account/recharge_points', 'RechargeController@postForm');
