...
class Language
{
    ...
    public function __construct($dataDir = null)
    {
        if (isset($dataDir)) {
            if (!is_dir($dataDir)) {
                die(sprintf('Unable to locate the Language data directory at "%s"', $dataDir));
            }
            $this->dataDir = $dataDir;
        } else {
            $this->dataDir = base_path('database/umpirsky/language');
        }
    }
     ...
     * Returns one country.
     *
     * @param string $languageCode The country
     * @param string $locale The locale (default: en)
     * @param string $source Data source: "icu" or "cldr"
	 * @return mixed|null
	 */
    public function get($languageCode, $locale = null, $source = null)
    {
        $languageCode = mb_strtoupper($languageCode);
        
        if (!$this->has($languageCode, $locale, $source)) {
            return null;
        }
     ...
     * the user to return an array with languages
    ...
    public function loadData($format = 'php', $locale = null, $source = null)
    {
        ...
        $this->languages = ($format == 'php') ? require($file) : file_get_contents($file);
        if (!is_array($this->languages)) {
            return [];
        }
        
        return $this->sortData($locale, $this->languages);
    }
     ...
     */
    public function has($languageCode, $locale = null, $source = null)
    {
        // Language Code
        if (empty($locale)) {
            $locale = config('app.locale');
        ...
        $languages = $this->all('php', $locale, $source);
        if (count($languages) <= 0) {
            return false;
        }
        $checker = isset($languages[mb_strtoupper($languageCode)]);
     ...
     */
    public static function transAll($languages, $locale = null, $source = null)
    {
        // Language Code
        if (empty($locale)) {
            $locale = config('app.locale');
        ...
        $tab = [];
        foreach ($languages as $code => $country) {
            $tab[$code] = $country;
