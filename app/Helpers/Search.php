...
class Search
{
    ...
    public function __construct($preSearch = [])
    {
		...
		if (isset($preSearch['countrycode']) && !empty($preSearch['countrycode'])) {
            $this->countrycode = $preSearch['countrycode'];
        }
		else
		{
       ...
       /* abdelhay 29-7-2020*/
        ...
        if(!empty($preSearch['photo']))
        {
           $this->arrSql->join[] = "JOIN " . DBTool::table('pictures') . " as pic ON pic.post_id=a.id"; 
        }
	   ...
	   $dis_c_point = 500;
        ...
        $me_g = " = '$cc' OR a.country_code = 'SA' OR a.country_code = 'IQ' OR a.country_code='QA' OR a.country_code = 'SY' OR a.country_code = 'JO'  OR a.country_code = 'BH' OR a.country_code = 'LB' OR a.country_code = 'PS' ";
        
        
        $ero_g = " = '$cc' OR a.country_code = 'DE' OR a.country_code = 'FR' OR a.country_code='IT' OR a.country_code = 'SP' OR a.country_code = 'ES' ";
        
        
         $ero_tr = " = '$cc' OR a.country_code = 'DE'  OR a.country_code = 'KW'";
        
        $usa_g = " = '$cc' OR a.country_code = 'US' OR a.country_code = 'CA' OR a.country_code='MX' ";
     ...
     /* abdelhay 29-7-2020
        ...
        if(!empty($preSearch['distance']))
        {
            ...
            else
            {
               ...
               if($preSearch['distance'] <= 19999 )
            { 
                
                $this->arrSql->where = [
                 '(a.country_code' 	=> $serchINC.')',
                '(a.verified_email' => " = 1 AND a.verified_phone = 1)",
                'a.archived'     	=> " = 0",
               // 'a.deleted_at'   	=> " IS NULL",
                    ];
    ...
    public function fetch()
    {
        ...
        // DB::enableQueryLog();
		// (self::query($sql, $this->bindings, 0));
		// dd(DB::getQueryLog());
        // dd($sql);
	...
	public function fetch_app()
    {
        ...
        return ['paginator' => $paginator->getCollection(), 'count' => $count];
    }
    ...
    public function setCategory($catId, $subCatId = null)
    {
        ...
        if (empty($subCatId))
        {
            ...
            $IDsList = $this->sGetAllSubs($catId);
            $catId = '(a.category_id='.$catId;
            foreach($IDsList as $key){
                $catId .= ' or a.category_id='.$key;
            }
        //    $catId = 'category_id='.$subCatId.' and';
           $catId .= ')';
            // $this->arrSql->where['a.category_id'] = ' = 17475';
            $this->arrSql->where[] = $catId;
        ...
        else
        {
            ...
            $IDsLists = $this->sGetAllSubs($subCatId);
            $IDsList = '(a.category_id='.$subCatId;
            foreach($IDsLists as $key){
                $IDsList .= ' or a.category_id='.$key;
            }
            $IDsList .= ')';
            $this->arrSql->where[] = $IDsList;
            // dd($IDsList);
        }
    ...
    public function sGetAllSubs($id){
        $IDsss = array();
        $catsss = Category::where('parent_id', $id)->get();
        foreach ($catsss as $cat){
            $IDsss[] = $cat->id;
            $catChildren = Category::where('parent_id', $cat->id)->get();
            if (count($catChildren)){
                $IDsss = array_merge($IDsss, $this->sGetAllSubs($cat->id));
            }
        }
        return $IDsss;
    }
