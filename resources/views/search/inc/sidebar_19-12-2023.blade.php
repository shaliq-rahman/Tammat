...
25-6-2020
abdelhay notes
1-width for <a href>  it's effected in + will not work if take full width
-->

<?php
// abdelhay comment   || 1==1 
$postsCount=0;
if(empty(Request::get('distance')))
{$Request_get_distance=300;}
else{
    $Request_get_distance=Request::get('distance');
}
...
//->where(function ($query) use ($subchildIds,$childIds,$superid,$superid_ext) {
	//->orWhereIn('category_id',[$superid,$superid_ext]);
                         ...
                         <?php /*?>    <a href="https://www.tmmat.com/category/<?php print_r($supersubcat_3[$w]->slug); ?>/<?php print_r($supersubcat_4[$z]->slug); ?>" title="{{$supersubcat_4[$z]->name}}">
        <?php */?>
                                  ...
                                  // echo "xxxx".$xxx=Request::get('distance');
									...
									@if (isset($isCitySearch) and $isCitySearch and \App\Helpers\DBTool::checkIfMySQLFunctionExists(config('larapen.core.distanceCalculationFormula')))
										...
										?>
                                            ...
                                            <?php if($Request_get_distance==300 && $iDist==300) {?>  selected="selected"<?php }?>
										...
										<?php if(empty($Request_get_distance)) {?>  selected="selected"<?php }?>
