...
// 			})
    ...
    public function updateDetails(UpdateUserRequest $request)
	{
		...
		$phoneChanged = ($request->phone_number != $user->phone);
...
protected function check_fav_post($post_id,$user_id){
    ...
    {
		$user_id = $request->user_id;
		$favouriteUsers =  SavedUser::leftjoin('users','users.id','saved_users.fav_user_id')->where('user_id','=',$user_id)->get();
    ...
    public function getmyads(Request $request)
    {
	 ...
	 if(!empty($request->ads_type) && $request->ads_type=='archive'){
      ...
      ->orderByDesc('id')->get();
        ...
        return response()->json(['results'=>$postimg]);
    }
...
public function unarchivepost(Request $request)
    {
           ...
           $postId = $request->id;
            ...
            if (!$res) {
                $Success=true;
                $result=t("The repost has done successfully");
            } else {
                $Success=false;
                $result=t("Please try again.");
            }
