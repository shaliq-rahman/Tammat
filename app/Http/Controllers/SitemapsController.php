...
class SitemapsController extends FrontController
{
	...
	public function index()
	{
		foreach ($this->countries as $item) {
			...
			Sitemap::addSitemap(url($country->locale . '/' . $country->icode . '/sitemaps.xml'));
		}
	 ...
	 */
	public function site($countryCode)
	{
		// Get Country Settings
		...
		Sitemap::addSitemap(url($country->locale . '/' . $country->icode . '/sitemaps/pages.xml'));
		Sitemap::addSitemap(url($country->locale . '/' . $country->icode . '/sitemaps/categories.xml'));
		Sitemap::addSitemap(url($country->locale . '/' . $country->icode . '/sitemaps/cities.xml'));
		
		$countPosts = Post::verified()->countryOf($country->code)->count();
		if ($countPosts > 0) {
			Sitemap::addSitemap(url($country->locale . '/' . $country->icode . '/sitemaps/posts.xml'));
		}
	 ...
	 * @return bool|\Illuminate\Http\Response
	 */
	public function pages($countryCode)
	{
		// Get Country Settings
		$country = $this->getCountrySettings($countryCode);
		if (empty($country)) {
	 ...
	 * @return bool|\Illuminate\Http\Response
	 */
	public function categories($countryCode)
	{
		// Get Country Settings
		$country = $this->getCountrySettings($countryCode);
		if (empty($country)) {
			...
			foreach ($cats as $cat) {
				...
				if ($subCats->get($cat->id)) {
					foreach ($subCats->get($cat->id) as $subCat) {
						...
						Sitemap::addTag($url, $this->defaultDate, 'weekly', '0.5');
					}
				}
			}
		}
	 ...
	 * @return bool|\Illuminate\Http\Response
	 */
	public function posts($countryCode)
	{
		// Get Country Settings
		$country = $this->getCountrySettings($countryCode);
		if (empty($country)) {
