...
class PhotoController extends FrontController
{
	...
	public function postForm_app($postIdOrToken, PhotoRequest $request)
    {
     ...
     //echo 'tmpPostId='.$request->tmpPostId;
        
        // Get Post
        if (getSegment(2) == 'create') {
            if (!$request->tmpPostId) {
                if ($request->ajax()) {
       ...
       $checkpaymentpayaccount = \DB::table('payments')->where('post_id', '=', $post->id)->where('active', '=', 1)->count();
        ...
        $checkpicturecount = \DB::table('pictures')->where('post_id', '=', $post->id)->count();
     ...
     * @return \Illuminate\Http\JsonResponse|\Illuminate\Http\RedirectResponse|\Illuminate\Routing\Redirector
     */
    public function delete_app($postIdOrToken, $pictureId, Request $request)
    {
        $inputs = $request->all();
        ...
        if (getSegment(2) == 'create') {
            ...
            $post = Post::withoutGlobalScopes([VerifiedScope::class, ReviewedScope::class])->where('id', $request->tmpPostId)->where('tmp_token', $postIdOrToken)->first();
        } else {
            $post = Post::withoutGlobalScopes([VerifiedScope::class, ReviewedScope::class])->where('user_id', $request->user_id)->where('id', $postIdOrToken)->first();
        }
        ...
        return response()->json(['results' => 'The picture has been deleted.']);
