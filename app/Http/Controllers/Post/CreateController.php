...
foreach($customArr as $custom){
	...
	if ($request->hasFile('img1')) {
        ...
        $name = time().'.'.$image1->getClientOriginalExtension();
         $images[] = $name;
        $destinationPath = public_path('/storage/files/' .strtolower($country_code). '/' .$post->id);
        $image1->move($destinationPath, $name);
        $values = array('post_id' => $post->id,'filename' => 'files/' .strtolower($country_code). '/' .$post->id. '/' .$name,'position'=>1,'active'=>1,'created_at'=>date("Y-m-d h:i:s a", time()),'updated_at'=>date("Y-m-d h:i:s a", time()));
        DB::table('pictures')->insert($values);
        }
        else{
           $image = ''; 
        }
    ...
    if ($request->hasFile('img2')) {
        ...
        if($image2){
        $name = time().'1.'.$image2->getClientOriginalExtension();
         $images[] = $name;
        $destinationPath = public_path('/storage/files/' .strtolower($country_code). '/' .$post->id);
    ...
    if ($request->hasFile('img7')) {
        ...
        if($image){
        $name = time().'6.'.$image->getClientOriginalExtension();
        $images[] = $name;
        $destinationPath = public_path('/storage/files/' .strtolower($country_code). '/' .$post->id);
        $image->move($destinationPath, $name);
        $values = array('post_id' => $post->id,'filename' => 'files/' .strtolower($country_code). '/' .$post->id. '/' .$name,'position'=>1,'active'=>1,'created_at'=>date("Y-m-d h:i:s a", time()),'updated_at'=>date("Y-m-d h:i:s a", time()));
    ...
    if ($request->hasFile('img8')) {
        ...
        if($image){
        $name = time().'7.'.$image->getClientOriginalExtension();
        $images[] = $name;
        $destinationPath = public_path('/storage/files/' .strtolower($country_code). '/' .$post->id);
        $image->move($destinationPath, $name);
        $values = array('post_id' => $post->id,'filename' => 'files/' .strtolower($country_code). '/' .$post->id. '/' .$name,'position'=>1,'active'=>1,'created_at'=>date("Y-m-d h:i:s a", time()),'updated_at'=>date("Y-m-d h:i:s a", time()));
    ...
    if ($request->hasFile('img9')) {
        ...
        if($image){
        $name = time().'8.'.$image->getClientOriginalExtension();
        $images[] = $name;
        $destinationPath = public_path('/storage/files/' .strtolower($country_code). '/' .$post->id);
        $image->move($destinationPath, $name);
        $values = array('post_id' => $post->id,'filename' => 'files/' .strtolower($country_code). '/' .$post->id. '/' .$name,'position'=>1,'active'=>1,'created_at'=>date("Y-m-d h:i:s a", time()),'updated_at'=>date("Y-m-d h:i:s a", time()));
        ...
        if($image){
        $name = time().'9.'.$image->getClientOriginalExtension();
        $images[] = $name;
        $destinationPath = public_path('/storage/files/' .strtolower($country_code). '/' .$post->id);
        $image->move($destinationPath, $name);
        $values = array('post_id' => $post->id,'filename' => 'files/' .strtolower($country_code). '/' .$post->id. '/' .$name,'position'=>1,'active'=>1,'created_at'=>date("Y-m-d h:i:s a", time()),'updated_at'=>date("Y-m-d h:i:s a", time()));
        ...
        $image = $request->file('img11');
        if($image){
        $name = time().'10.'.$image->getClientOriginalExtension();
        $images[] = $name;
        $destinationPath = public_path('/storage/files/' .strtolower($country_code). '/' .$post->id);
        $image->move($destinationPath, $name);
        $values = array('post_id' => $post->id,'filename' => 'files/' .strtolower($country_code). '/' .$post->id. '/' .$name,'position'=>1,'active'=>1,'created_at'=>date("Y-m-d h:i:s a", time()),'updated_at'=>date("Y-m-d h:i:s a", time()));
        ...
        $image = $request->file('img12');
        if($image){
        $name = time().'11.'.$image->getClientOriginalExtension();
        $images[] = $name;
        $destinationPath = public_path('/storage/files/' .strtolower($country_code). '/' .$post->id);
        $image->move($destinationPath, $name);
        $values = array('post_id' => $post->id,'filename' => 'files/' .strtolower($country_code). '/' .$post->id. '/' .$name,'position'=>1,'active'=>1,'created_at'=>date("Y-m-d h:i:s a", time()),'updated_at'=>date("Y-m-d h:i:s a", time()));
		...
		$post['package_id'] = $request->input('package_id');
        // Check if Payment is required
        $package = Package::find($request->input('package_id'));
        /*if (!empty($package)) {
            if ($package->price > 0 && $request->filled('payment_method_id') && !$alreadyPaidPackage) {
