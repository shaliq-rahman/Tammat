...
class PaymentController extends FrontController
{
    ...
    public function postForm($postIdOrToken, PackageRequest $request)
    {
      ...
      $query_update =  \DB::table('posts')
        ...
        if (getSegment(2) == 'create') {
            ...
            $nextStepUrl = config('app.locale') . '/posts/create/' . $postIdOrToken . '/photos';
        } else {
     ...
     * @return \Illuminate\Http\RedirectResponse|\Illuminate\Routing\Redirector
     */
    public function postForm_app($postIdOrToken, PackageRequest $request)
    {
        // Get Post
        if (getSegment(2) == 'create') {
      ...
      $query_update =  \DB::table('posts')
        ...
        if (!empty($package)) {
            if ($package->price > 0 && $request->filled('payment_method_id') && !$alreadyPaidPackage) {
                // Send the Payment
                //$this->sendPayment($request, $post);
				return response()->json(['results'=>'Payment has been done.']); 
            }
        }
