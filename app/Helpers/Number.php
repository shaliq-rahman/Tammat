...
class Number
{
	...
	public static function money_price_latest($number,$currency,$in_left,$decimal_places,$decimal_separator)
	{
	    if (config('settings.other.decimals_superscript')) {
			return static::moneySuperscript($number);
		}
        ...
        // Remove decimal value if it's null
		$defaultDecimal = str_pad('', (int)$decimal_places, '0');
		$number = str_replace($decimal_separator . $defaultDecimal, '', $number);
	...
	public static function toFloat($number)
	{
		...
		$sepPos = (($dotPos > $commaPos) && $dotPos) ? $dotPos : ((($commaPos > $dotPos) && $commaPos) ? $dotPos : false);
