...
class CategoryController extends PanelController
{
	...
	public function setup()
	{
		...
		*/
		...
		$this->xPanel->setRoute(config('larapen.admin.route_prefix', 'admin') . '/categories');
    ...
    { 
        ...
        $output = implode("\t", array('ID', 'Name','Lang.')) . "\r\n";
        $i = 0;
        foreach ($users as $row) {
            ...
            $output .= implode("\t", array($row->id, $row->name, $row->translation_lang)) . "\r\n"; // append each row
        }
        $headers = array(
            'Content-Type' => 'text/csv',
            'Content-Disposition' => 'attachment; filename="allcats_' . date('Y-m-d') . '.xls"',
        );
