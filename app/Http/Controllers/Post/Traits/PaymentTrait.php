...
trait PaymentTrait
{
     ...
     * @return \Illuminate\Http\RedirectResponse|\Illuminate\Routing\Redirector
     */
    public function sendPaymentApp(Request $request, Post $post)
    {
        // Set URLs
        $this->uri['previousUrl'] = str_replace(['#entryToken', '#entryId'], [$post->tmp_token, $post->id], $this->uri['previousUrl']);
        
            // Load Payment Plugin
            $plugin = load_installed_plugin('paypal');
    ...
    public function paymentConfirmation($postIdOrToken)
    {
        ...
        $postData = DB::table('posts')->where('id', $params['post_id'])->first();
        ...
        {
            $res = call_user_func($plugin->class . '::paymentConfirmation', $params, $post);
            if($res){
            return redirect('/?success=paymentConfirmation&deviceType=mobile');
            }
        }
        else{
