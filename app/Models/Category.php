...
class Category extends BaseModel
{
	...
	*/
	...
	public function getCheldFoCategoryByID($id)
	{
	...
	//	if ($this->parent_id == 0) {
		    if (true) {
			$url = url(config('larapen.admin.route_prefix', 'admin') . '/categories/' . $this->id . '/subcategories');
	...
	{
		$url = url(config('larapen.admin.route_prefix', 'admin') . '/categories/' . $this->id . '/custom_fields');
