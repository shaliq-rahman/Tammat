...
@section('content')
    ...
    <div class="box box-primary">
        <div class="box-body">
            <button id="create-new-backup-button" href="{{ url(config('larapen.admin.route_prefix', 'admin').'/backups/create') }}" class="btn btn-primary ladda-button" data-style="zoom-in"><span class="ladda-label"><i class="fa fa-plus"></i> {{ trans('admin::messages.create_a_new_backup') }}</span></button>
            <br>
            ...
            <table class="table table-hover table-condensed">
                ...
                @foreach ($backups as $k => $b)
                    <tr>
                        <th scope="row">{{ $k+1 }}</th>
                        <td>{{ $b['disk'] }}</td>
                        <td>{{ \Date::createFromTimeStamp($b['last_modified'])->formatLocalized(config('larapen.admin.default_datetime_format')) }}</td>
