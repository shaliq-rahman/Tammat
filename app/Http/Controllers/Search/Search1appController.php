...
class Search1appController extends BaseController
{
	use PreSearchTrait;
	...
	public function index()
	{
	     ...
	     $getdetail = \DB::table('countries')
           ->select('*')
           ->where('code','=', request()->get('country'))
           ->first();
    	...
    	$city_query = \DB::table('cities')
	            ->select('*')
    	        ->where('country_code','=',request()->get('country'))
                ->where('name', 'like', '%'.$capital.'%')
                ->first();
	    
	    $locationid = !empty($city_query->id)?$city_query->id:'';
	    //echo 'locationid='.$capital;
	   // if(!empty($capital))
	   // {
    ...
    // 	    $capital = $capital[0];    
	   ...
	   // print_r(request()->all()); die;
		...
		//return response()->json(['isIndexSearch'=>$this->isIndexSearch]);
		// Pre-Search
		...
		return response()->json(['getdetail'=>$data]);
	}
