...
class FieldApp extends BaseModel
{
    ...
    */
	...
	public function optionsBtn($xPanel = false)
	{
		...
		if (isset($this->type) && in_array($this->type, ['select', 'radio', 'checkbox_multiple'])) {
			$url = url(config('larapen.admin.route_prefix', 'admin') . '/custom_fields/' . $this->id . '/options');
	...
	{
		$url = url(config('larapen.admin.route_prefix', 'admin') . '/custom_fields/' . $this->id . '/categories/create');
