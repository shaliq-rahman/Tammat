		...
		if (request()->filled('rejected')) {
			...
			if (request()->get('rejected') == 1) {
				...
				if (config('settings.single.posts_review_activation')) {
					...
					$this->xPanel->addClause('where', 'is_rejected', '=', 1);
...
// 		});
		...
		$this->xPanel->addFilter([
			...
			if ($value == 2) {
				...
				$this->xPanel->addClause('where', 'is_rejected', '=', 0);
		...
		*/
		...
		$this->xPanel->addColumn([
			...
			'function_name' => 'getIsRejectedHtml',
...
// 		]);
		...
		$this->xPanel->addColumn([
			...
			'label'         => 'Active',
			 ...
			 'function_name' => 'getActiveHtmlAjax',
		...
		$this->xPanel->addColumn([
			...
			'function_name' => 'getPendingHtmlAjax',
			'orderable' => true,
