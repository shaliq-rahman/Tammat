...
<p style="font-family: 'Helvetica Neue', 'Helvetica', Helvetica, Arial, sans-serif; font-size: 14px; line-height: 1.6em; font-weight: normal; margin: 0 0 10px; padding: 0; display: inline-block; max-width: 100%; word-wrap: break-word;"><?php $attr = ['slug' => slugify($post->title), 'id' => $post->id]; ?>
	<!--{!! nl2br($msg->message) !!}-->
	    ...
	    $delivery_preference = '';
	    $buyer_address = '';
	    if($msg->delivery_preference == '1')
	    {
	        $delivery_preference = t('Pick up the item myself');    
	    }
	    if($msg->delivery_preference == '2')
	    {
	        $delivery_preference = t('Shipped or deliverd to me');
	        $buyer_address = $msg->buyer_address;
	    }
	    ?>
	
	    <br><br>@lang('mail.post_seller_contacted_content_1', [
        'name'    => $msg->from_name,
    ...
    @lang('mail.post_seller_contacted_content_7')
        
    @lang('mail.post_seller_contacted_content_6')
