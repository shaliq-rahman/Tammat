...
<div class="modal fade" id="proceedDelivery" tabindex="-1" role="dialog">
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				...
				<h4 class="modal-title"><i class="icon-mail-2"></i> {{ t('Send Delivery Info') }} </h4>
			</div>
			<?php
			$post_id = !empty($post->id)?$post->id:'';
			?>
			<form role="form" method="POST" action="{{ lurl('posts/' .$post_id. '/contact') }}" id="proceedDeliveryForm" enctype="multipart/form-data">
				{!! csrf_field() !!}
				<div class="modal-body">
					...
					@endif
					    ...
					    <input type="hidden" name="offer_id" id="get_offer_id" value="{{ !empty($makeanoffer->id)?$makeanoffer->id:'' }}">
							...
							<div class="form-group required <?php echo (isset($errors) and $errors->has('from_email')) ? 'has-error' : ''; ?>">
								...
								<div class='col-md-6' style="padding: 0px;width: 97%;" id="buynowShowEmailusingCheckbox">
    								<div class="input-group">
    									...
    									<input id="buynowfrom_email" name="from_email" value="{{ old('from_email', auth()->user()->email) }}" type="text" placeholder="{{ t('i.e. you@gmail.com') }}"
    										   class="form-control" >
    								</div>
								</div>
								<div style="clear:both"></div>
					...
					<div class="form-group required <?php echo (isset($errors) and $errors->has('from_phone')) ? 'has-error' : ''; ?>">
						...
						<div class="col-md-6" id="buynowShowPhoneusingCheckbox" style="padding: 0px;width: 97%;">
    						<div class="input-group">
    							...
    							<input id="buynowfrom_phone" name="from_phone" value="{{ old('from_phone', (auth()->check()) ? auth()->user()->phone : '') }}" type="text"
					...
					<div class="form-group required <?php echo (isset($errors) and $errors->has('message')) ? 'has-error' : ''; ?>">
						...
						<textarea id="message" name="message" minlength="20" class="form-control required" placeholder="{{ t('Your message here...') }}" rows="5">{{ t('Hi, It is pleasure to make a deal with you. According to our deal agreement please check my delivery preference and my contact information here so we can proceed. Waiting for your response to take action.') }}</textarea>
					</div>
					...
					<div class="form-group required <?php echo (isset($errors) and $errors->has('date_time')) ? 'has-error' : ''; ?>">
						...
						<div class="col-md-6" style="padding: 0px;width: 97%;">
    						<div class="input-group">
    							...
    							<input  name="date_time" value="{{ old('date_time')}}" required type="text" placeholder="{{ t('Date & Time Preference') }}"  class="form-control datetimepicker"  >
    						</div>
						</div>
						<div style="clear:both"></div>
					</div>
					...
					<input type="hidden" name="delivery_info" value="1">
					<input type="hidden" name="country_code" value="{{ config('country.code') }}">
					<input type="hidden" name="post_id" id="get_post_id" value="{{ !empty($post->id)?$post->id:'' }}">
