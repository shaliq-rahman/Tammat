...
INNER JOIN post_values ON  posts.id = post_values.post_id
INNER JOIN post_types ON posts.post_type_id = post_types.id
WHERE '.$catId.' '.$field_values_custom.' '.$field_values.' '.$price_query.' '.$sallerType.' posts.country_code="'.$country.'" and posts.archived!=1  ORDER BY posts.price '.$order.'';
 ...
 //$postArray = json_decode(json_encode($posts),true);
    ...
    {   
        ...
        if(!empty($user)){
            ...
            $msg = "Clicking the Reset Password button, copy and paste the URL below into your web browser: https://www.dealnotdeal.com/password/reset/".$token;
            $from = "admin@dealnotdeal.com";
            $headers  = "From: $from\r\n";
            $headers .= "Content-type: text/html\r\n";
            if(mail($email,"Deal Not Deal Forgot Password",$msg,$headers)){
                $json['status'] = 1;
